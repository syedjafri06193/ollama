...
export function SidebarLayout({
  ...
  // Suppress the width/left transitions on initial render so the sidebar
  // appears immediately in its open/closed position rather than animating
  // in from the edge on every page load. Transitions are enabled only after
  // the component has mounted and persisted settings have been fetched.
  const [transitionsEnabled, setTransitionsEnabled] = useState(false);
  useEffect(() => {
    if (settingsData !== undefined) {
      // One rAF delay ensures the sidebar is already at its correct
      // position before CSS transitions are switched on.
  ...
  return (
    <div className={`flex dark:bg-neutral-900`}>
      <div
        className={`absolute flex mx-2 py-2 z-20 items-center text-neutral-500 dark:text-neutral-400 ${transitionsEnabled ? "transition-[left] duration-375" : ""} ${settings.sidebarOpen ? (isWindows ? "left-2" : "left-[204px]") : isWindows ? "left-2" : "left-20"}`}
      >
        <button
      ...
      <div
        className={`flex flex-col max-h-screen ${transitionsEnabled ? "transition-[width] duration-300" : ""} ${settings.sidebarOpen ? "w-64" : "w-0"}`}
      >
        <div
